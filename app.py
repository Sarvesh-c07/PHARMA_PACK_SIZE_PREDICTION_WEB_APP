...
def dashboard_body(dataset: Optional[WorkbookData]) -> str:
    ...
    def _cells(rs):
        ...
        return (f"<td>{uval(_med(rs, 'length_mm'), 'mm', 0)} × {uval(_med(rs, 'width_mm'), 'mm', 0)} × "
                ...
                f"<td>{fmt_number(md, ' g/cm\u00b3', 2) if md else '\u2014'}</td>")
    ...
    for ci, (c, n) in enumerate(cat_sorted):
        ...
        for si, (gk, gr) in enumerate(g_sorted[:8]):
            ...
            sid = f"{ci}-{si}"
            man_groups: Dict[str, List[Dict[str, Any]]] = {}
            for x in gr:
                mname = clean_text(x.get("manufacturer")) or "Unknown maker"
                man_groups.setdefault(mname, []).append(x)
            man_sorted = sorted(man_groups.items(), key=lambda kv: -len(kv[1]))
            rows.append(
                f'<tr class="sub-row str-row" data-parent="{ci}" data-strength="{sid}" hidden>'
                f'<td class="str-label"><span class="caret">▸</span>{h(gk)} <span class="small">· {len(man_sorted)} makers</span></td>'
                f'<td>{fmt_int(len(gr))}</td>'
                f'{_cells(gr)}'
                f'<td class="cov-cell"><span class="cov-badge {gcov["cls"]}">{h(gcov["label"])}</span></td>'
                '</tr>'
            )
            for mk, mr in man_sorted[:8]:
                rows.append(
                    f'<tr class="subsub-row" data-sparent="{sid}" hidden>'
                    f'<td class="man-label">{h(_short(mk, 36))}</td>'
                    f'<td>{fmt_int(len(mr))}</td>'
                    f'{_cells(mr)}'
            ...
            if len(man_sorted) > 8:
                extra = man_sorted[8:]
                extra_recs = [x for _, mr in extra for x in mr]
                rows.append(
                    f'<tr class="subsub-row" data-sparent="{sid}" hidden>'
                    f'<td class="man-label"><span class="small">{len(extra)} other manufacturers</span></td>'
    ...
    summary_table = f"""
    <div class="drill-hint">▸ <b>Click a category</b> to open it by strength / dosage, then <b>click a strength</b> to see the manufacturers making it.</div>
    <div class="table-wrap"><table class="drill">
      <thead><tr><th>Category / strength</th><th>Packs</th><th>Median L × W × H</th><th>Median weight</th><th>Median density</th><th>Model coverage</th></tr></thead>
      <tbody>{''.join(rows)}</tbody>
    </table></div>
    """
...
def main() -> None:
    ...
    print(f"Pharma Pack Dimension Predictor running on {host}:{port}")
